    ...
    <table id="inputserviceplace" style="background: #fffeee">
        <tr>
            <td id="hoadoninfo">
                ...
                <? endif; ?>
                <br>
                <input tabindex="1" type="text" name="pgmahoadon" style="width:40%;display: inline-block"
                       placeholder="Mã hóa đơn">
                <input tabindex="2" type="text" id="pgdate" name="pgdate" style="width:30%;display: inline-block"
                       placeholder="Ngày">
                <input tabindex="3" type="text" id="pghour" name="pghour" style="width:25%;display: inline-block"
        ...
        <tr>
            <td colspan="2">
                <div>Thông tin Sản phẩm: mã <b id="icode"></b>, tên SP: <b id="iname"></b>, loại: <b id="itype"></b>,
                    tồn kho <b id="icount"></b></div>
            </td>
        </tr>
        <tr>
            <td colspan="2">
                ...
                <input tabindex="6" type="text" onblur="" name="pgprice" style="width:15%;display: inline-block"
                       placeholder="Giá">
                <label style="width:5%">S/Lg</label>
                <input tabindex="7" type="text" name="pgcount" style="width:5%;display: inline-block"
        ...
        <tr>
            <td colspan="2">
                <div style="display: inline-block;float:left">
                    <label>Sale</label>
                    <input type="text" name="pgsaleamount" value="" id="pgsaleamount" style="width:40%"
                           placeholder="số giảm">
                    <select name="pgsaleunit" style="width:20%">
                        <option value="percent">%</option>
                        <option value="abs">đ</option>
                    </select>
                </div>
                <input type="hidden" name="idhoadon" value="">
                <input type="hidden" name="idchitiethoadon" value="">
                    <span class="btn btn-follow"><input tabindex="10" type="button" value="Lưu"
                                                        onclick="save()"> </span>
    ...
    <table style="background: #fff3f4">
        <tr>
            <td id="thanhtoanbox" style="width:40%">
                <div>
                    <label style="width: 50%">Tổng giá trị hóa đơn: </label> <b id="pgsumprice"
                                                                                style="text-align: right">0</b><br>
                    <label style="width: 50%">Số tiền còn lại: </label> <b id="pgsumremain"
                                                                           style="text-align: right">0</b><br>
                </div>
                <div>
                    <label style="width: 50%">Thanh toán: </label> <br>
                    <input type="hidden" name="pgtypethanhtoan">
                    <input type="text" name="pgthanhtoan" style="width:80%;display: block"
...
<fieldset style="float:left;width:59%">
    <legend>Chi tiết hóa đơn</legend>
    <div id="list_hoadonitem"></div>
</fieldset>
<fieldset style="float:right;width:39%">
    ...
    <div style="float:left;width: 90%">
        <input type="text" name="searchinout" id="searchinout" style="width: 80%" placeholder="Mã hóa đơn">
    ...
    $.ajax({
        ...
        success: function (msg) {
            if (msg == 0) {
                ...
                var pgsaleamount = $("input[name=pgsaleamount]").val().replace(/ /g, '');
                var pgsaleunit = $("select[name=pgsaleunit]").val();
...
//                            + "&pgyear=" + pgyear
                            ...
                            + "&pgcount=" + pgcount
                            + "&pginout_id=" + idhoadon
                            + "&pgsaleunit=" + pgsaleunit
                            + "&pgsaleamount=" + pgsaleamount
    ...
    $.ajax({
        ...
        success: function (msg) {
            if (msg == "0") alert('<?=lang("NO_DATA")?>');
                ...
                $("input[name=pgsaleamount]").val(province.pgsaleamount);
                $("select[name=pgsaleunit]").val(province.pgsaleunit);
...
function checkXuat(sn, xuattype, from, to, inout_id) {
    $.ajax({
        ...
        success: function (msg) {
            switch (msg) {
                case "-1":
                    alert("Sản phẩm đã bán cho khách");
                    $("input[name=pgseries]").val("");
                    break;
                case "-2":
                    alert("Sản phẩm đã ở cửa hàng này hoặc đang được chuyển vào");
                    $("input[name=pgseries]").val("");
                    break;
                case "-3":
                    alert("Sản phẩm không có ở cửa hàng này hoặc đang được chuyển đi");
                    $("input[name=pgseries]").val("");
                    break;
                case "-4":
                    alert("Nơi chuyển và nơi nhận phải khác nhau");
                    $("input[name=pgseries]").val("");
                    break;
                case "-5":
                    alert("Chưa chọn nơi chuyển và nơi nhận");
                    $("input[name=pgseries]").val("");
                    break;
                case "-6":
                    alert("Đã có sản phẩm này trong đơn hàng");
                    $("input[name=pgseries]").val("");
                    break;
                case "-11":
                    alert("Hàng không có trong kho");
                    $("input[name=pgseries]").val("");
                    break;
                case "1":
                    break;
...
function savethanhtoan() {
    ...
    switch (pgtypexuat) {
        ...
        case 'thuhoi':
            ...
            break;
        case 'xuatkho':
            storeid = $("select[name=pgfrom]").val();
